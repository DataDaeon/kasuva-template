    ...
    protected function _resizeProductImage($product, $type, $keepFrame, $image = null, $placeholder = false)
    {
        ...
        if ($hasTypeData || $placeholder || $image) {
            ...
            if ($type == 'small_image') {
				...
				$_imgWidth=($Cfg['catprodimg_width'])? $Cfg['catprodimg_width'] : 270;
				$_imgHeight=($Cfg['catprodimg_height']) ? $Cfg['catprodimg_height'] : 335;
				if($_imgHeight<=0){
					$helper->constrainOnly(false)->keepAspectRatio(true)->keepFrame(false)->resize($_imgWidth);
				}else{
					$helper->constrainOnly(true)->resize($_imgWidth, $_imgHeight);
				}
            }else{
